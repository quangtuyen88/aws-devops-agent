...
## Code-Generation Expansion (appended 2026-06-17, construction/UNIT-001)

> Owner: aidlc-sw-dev-engineer-agent. Everything above is copied forward from
> `infrastructure-design/unit.md` UNCHANGED — packaging, boundaries, internal modules, and
> the functional/infrastructure expansions are preserved verbatim. This section records the
> realised code structure. No packaging or boundary decision is changed.

### Realised package (modular monolith, one artifact → three Lambda roles)

```
src/slack_devops_agent/
  domain/        # ENT-001..014 (pydantic), ProcessingJob FSM, pure BR-001..027 logic
  ports/         # API-INT-001..009 Protocols (the stable seams)
  config/        # env-based Settings (pydantic-settings)
  resilience/    # backoff (NFR-15), circuit breaker (NFR-16), time budget (NFR-17), clock
  observability/ # structured JSON logging + redaction (NFR-6), EMF metrics (NFR-20)
  components/
    intake/      # CMP-001 (W1 + W4)
    worker/      # CMP-002 (W2/W3) + composer + rendering
    inference/   # CMP-003 (kiro_gateway HTTP-client-only + bedrock alternate)
    mcp/         # CMP-004
    safety/      # CMP-005
    jobs/        # CMP-006 (DynamoDB)
    opdata/      # CMP-007 (DynamoDB)
    config/      # CMP-008 (DynamoDB)
    queue/       # API-INT-009 SQS producer (C-1 seam)
    recovery/    # reaper (F3 / W3)
  entrypoints/   # lambda_intake, lambda_worker, lambda_reaper + dispatch + wiring
tests/           # mirrors src/; moto for bounded-context, respx + fakes for live deps
```

### Decisions applied (human-answered 2026-06-17, code-generation/questions.md)

- **Q1=a** — application code + tests only; Terraform IaC deferred to a follow-on slice.
- **Q2=a** — F8 mapping = `answer_message_ts` attribute on `ProcessingJob` + `answer-ts-index`
  GSI; stable ENT-008 ID preserved, no new entity (flagged for systems-architect confirmation).
- **Q3=a** — Kiro-gateway primary integrated HTTP-client-only; gateway never vendored/forked/
  imported (AGPL bounded); Bedrock alternate one `INFERENCE_BACKEND` flip away.
- **Q4=a** — `moto` for bounded-context DynamoDB/SQS tests; injected clock for NFR-19/F5 timing
  races; live Slack/Kiro/Bedrock/MCP behind typed fakes/respx.

### Verification (full suite, all green)

`ruff check` · `ruff format --check` · `mypy` (strict) · `pytest` (95 passed) ·
`bandit -r src/` (0 issues). See `implementation-map.md` for full ID→file/test traceability.

### Boundaries retained (unchanged)

OOS-3 (never executes/deploys IaC — BR-017), A-8/OOS-4 (no durable conversation memory;
fetch-not-store — BR-005), A-5/OOS-5 (single workspace), NFR-5 (least-privilege creds at
every external surface; secrets from env injected by Secrets Manager at runtime — never
hardcoded). The AGPL `kiro-gateway` watch-item is closed for this pass (HTTP-client-only).
